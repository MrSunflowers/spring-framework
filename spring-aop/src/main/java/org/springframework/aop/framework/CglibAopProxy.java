...
class CglibAopProxy implements AopProxy, Serializable {
	 ...
	 * 以了，比如将 advised 属性封装在 DynamicAdvisedInterceptor 并加入在 callbacks 中，这么做的
	 * 目的是什么呢，如何调用呢？在前面的示例中，我们了解到 CGLIB 中对于方法的拦截是通过
	 * 将自定义的拦截器（实现MethodInterceptor 接口）加入Callback 中并在调用代理时直接激活拦
	 * 截器中的 intercept 方法来实现的,那么在getCallback 中正是实现了这样一个目的,DynamicAdvisedlnterceptor
	 * 继承自Methodlnterceptor ，加入Callback 中后，在再次调用代理时会直接调
	 * 用 DynamicAdvisedlnterceptor 中的 intercept 方法，由此推断，对于CGLIB 方式实现的代理，
	 * 其核心逻辑必然在 DynamicAdvisedlnterceptor 中的 intercept 中
	 */
	 ...
	 * proxy is not frozen.
	 * <p>实现与JDK 方式实现代理中的invoke 方法大同小异，都是首先构造链，然后封装
	 * 此链进行串联调用，稍有些区别就是在JDK 中直接构造ReflectiveMethodlnvocation ，而在cglib
	 * 中使用 CglibMethodlnvocation 。CglibMethodlnvocation 继承自 ReflectiveMethodlnvocation ， 但
	 * 是proceed方法并没有重写
	 */
